...
func dockerNet(cmd *cobra.Command, args []string) {
	...
	for _, container := range containerRes.Containers {
		...
		if outStr, errStr, err := goblin.CmdOutErr("/usr/bin/nsenter", "-t", pid, "-n", "/bin/sh", "-c", cmdStr); err != nil {
			fmt.Println(goblin.AddPrefix(color.HiBlueString("[%s]", pid), outStr))
			fmt.Println(goblin.AddPrefix(color.HiRedString("[%s]", pid), errStr))
		}
	}
